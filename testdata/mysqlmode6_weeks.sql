...
CREATE TEMPORARY TABLE IF NOT EXISTS Mode6Weeks1 AS
SELECT ROW_NUMBER() over w AS rownum,
       year,
       week,
       start
FROM (SELECT CAST(LEFT(YEARWEEK(d1.dt, 6), 4) AS UNSIGNED) as year,
             WEEK(d1.dt, 6) as week,
             d1.dt as start
      FROM d1
               INNER JOIN d0 ON d0.dt = (d1.dt - INTERVAL 1 DAY)
      where YEARWEEK(d0.dt, 6) != YEARWEEK(d1.dt, 6)
      ORDER BY d1.dt) as weeks
WINDOW w AS (ORDER BY start);

CREATE TEMPORARY TABLE IF NOT EXISTS Mode6Weeks2 AS SELECT * FROM Mode6Weeks1;
...
FROM Mode6Weeks1 w1
         INNER JOIN Mode6Weeks2 w2 on w1.rownum = (w2.rownum -1);