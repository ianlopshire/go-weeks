...
CREATE TEMPORARY TABLE IF NOT EXISTS Mode2Weeks1 AS
SELECT ROW_NUMBER() over w AS rownum,
       year,
       week,
       start
FROM (SELECT YEAR(d1.dt)                         as year,
             WEEK(d1.dt, 2)                      as week,
             d1.dt as start
      FROM d1
               INNER JOIN d0 ON d0.dt = (d1.dt - INTERVAL 1 DAY)
      where WEEK(d0.dt, 2) != WEEK(d1.dt, 2)
      ORDER BY d1.dt) as weeks
WINDOW w AS (ORDER BY start);

CREATE TEMPORARY TABLE IF NOT EXISTS Mode2Weeks2 AS SELECT * FROM Mode2Weeks1;
...
FROM Mode2Weeks1 w1
         INNER JOIN Mode2Weeks2 w2 on w1.rownum = (w2.rownum -1);